...
public interface SitterMapper {
	// 목록
	// 전체 목록
	@Select("SELECT sitter_no, jjimcount, carecount, score, tag, content, sitter_pic, care_loc, pet_first_price, nickname, user_name, num " 
		      + "FROM (SELECT sitter_no, jjimcount, carecount, score, tag, content, sitter_pic, care_loc, pet_first_price, nickname, user_name, rownum AS num " 
		      + "FROM (SELECT ps.sitter_no, ps.jjimcount, ps.carecount, ps.score, ps.tag, ps.content, ps.sitter_pic, ps.care_loc, ps.pet_first_price, "
		      + "pu.nickname, pu.user_name "
	...
	@Select("SELECT s.* "
			...
			+ "FROM p_sitter s JOIN p_users u ON s.user_no = u.user_no "
