...
public interface GroupFeedMapper {
	...
	@Update("UPDATE p_feed SET title=#{title}, content=#{content} WHERE feed_no=#{feed_no}")
	public void feedUpdate(FeedVO vo);
	
	@Delete("DELETE FROM p_feed WHERE feed_no = #{feed_no}")
	public void feedDelete(int feed_no);
	@Delete("DELETE FROM p_feed_comment WHERE feed_no = #{feed_no}")
	public void feedCommentTableDelete(int feed_no);
	@Delete("DELETE FROM p_feed_fileinfo WHERE feed_no = #{feed_no}")
	public void feedFileInfoTableDelete(int feed_no);
	@Delete("DELETE FROM p_feed_like WHERE feed_no = #{feed_no}")
