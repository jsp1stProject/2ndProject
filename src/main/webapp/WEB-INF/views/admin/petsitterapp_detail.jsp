    ...
    <div class="col-12">
        <c:choose>
            ...
            <div class="d-flex align-items-center justify-content-between p-3">
                <div>
                    <a href="${pageContext.request.contextPath}/admin/users/${result.user_no}" class="text-decoration-none">
                        <c:choose>
                 ...
                 <div>
                     <c:choose>
                         <c:when test="${result.status eq 0}">
                             <button type="button" class="btn btn-sm btn-info" onclick="appExecute(1)">허가하기</button>
                             <button type="button" class="btn btn-sm btn-danger" onclick="appExecute(2)">반려하기</button>
                         </c:when>
                     </c:choose>
...
<script>
    async function appExecute(status){
        console.log(status)
        try{
            const res = await axios({
                method:'post',
                url:'${pageContext.request.contextPath}/api/admin/petsitters/${result.user_no}/applications',
